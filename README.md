...
アプリと書かれた横にあるラジオボタンから削除・無効化・回復のいずれかを選び、  __検索__ と書かれたボタンを押すことで選択されたデバイスのキャリアアプリを検索します。
...
|回復|アンインストールしたキャリアアプリを再度インストールします|
...
回復の場合は回復したいアプリを選択してください。



また、Wi-Fiアプリ・災害系のアプリを除外するのボタンを押すことで該当のアプリの選択を解除することができます。
(回復の場合はWi-Fiアプリ・災害系アプリを追加するボタンに変わります。)

なお、2022年7月24日時点でのWi-Fiアプリ・災害系のアプリですので今後変更された場合には除外・追加されない可能性がありますので、あくまで __目安__ としてご使用ください。

#### 処理を実行
削除を実行・無効化を実行・回復を実行のいずれかのボタンを押すことでチェックボックスのチェックのついているアプリをそれぞれ削除(アンインストール)・無効化・回復(再インストール)します。
